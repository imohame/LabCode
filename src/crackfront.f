...
!!!      parameter (nume=40000)
     ...
     1       nodeflag(4,nume)
	  ...
	  do ele=1,numelto
	      ...
	      if(elecrack(2,ele)==1) then
			  ...
			  if(bflag==1) then 
			      if((elecrack(1,ele2)==ne2 .and. elecrack(2,ele2)==2)
     >              .or. 
     >              (elecrack(3,ele2)==ne2 .and. elecrack(4,ele2)==2))
     >              then
	                  ntp=ntp+1
                      tipele(1,ntp)=ele
				      tipele(2,ntp)=ele2
     ...
     >              then
	  ...
	  do i=1, ncrack   ! save cracked elements information 
	                   ! to calculate crack velocity
		  ...
		  if(llast==1) then
	          do j=1,ntp
		          if(tipelenum(nlc, llast)==tipele(1,j)) then
				      ele1=tipele(1,j)
				      ele22=tipele(2,j)
				      yele1=(y(ix(1,ele1))+y(ix(2,ele1))
     >					    +y(ix(3,ele1))+y(ix(4,ele1)))/4.0
	                  yele2=(y(ix(1,ele22))+y(ix(2,ele22))
     >					    +y(ix(3,ele22))+y(ix(4,ele22)))/4.0
