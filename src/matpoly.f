...
!    >                 tau(nss), p(nss,3), taur, twomu, g                   !OLD WAY
     ...
     > sig12(nelemg)      
      common/vect3/
     >      dgi1(nelemg,4),dgi2(nelemg,4),dgi3(nelemg,4),
     > dgi4(nelemg,4),dgi5(nelemg,4),
     >      dgt1(nelemg,4),dgt2(nelemg,4),dgt3(nelemg,4),
     > dgt4(nelemg,4),
     >      f11v(nelemg),f22v(nelemg),f12v(nelemg),f21v(nelemg),
     > dsd5(nelemg),
     >      sig11s(nelemg),sig22s(nelemg),sig33s(nelemg),
     > sig12s(nelemg),
     >      ddp1(nelemg,4),ddp2(nelemg,4),ddp3(nelemg,4),
     > ddp4(nelemg,4),ddp5(nelemg,4)
      common/bk08/    kprint,nstep,ite,ilimit,newstf
      common/bk32/    nsref,nequit,time,timep,lprint,nprint
      common/meng/
     >		f11dd(nelemg),f22dd(nelemg),f12dd(nelemg),f21dd(nelemg),
     > f33dd(nelemg),      !F matrix
     >		f11dv(nelemg),f22dv(nelemg),f12dv(nelemg),f21dv(nelemg),
     > f33dv(nelemg),      !Fdot matrix
     >	 vg11(nelemg),vg22(nelemg),vg12(nelemg),vg21(nelemg),
     > vg33(nelemg),fdet(nelemg),    !FdotFinverse=velocity Gradient
     ...
     > sign4(nelemg)
      ...
      common/variab/  aaa1(nume),aaa2(nume),aaa3(nume),aaa4(nume),
     >                aaa5(nume),aaa6(nume),aaa7(nume)
      common/cccc/    ntt
...
!!c	  common /gbtranr/ gbtr(14)
     ...
     > slip_sgb(24,3)
	  dimension bres(24,24),cb(24,24),sb(24,24),cnu(24,24),
     > snu(24,24)
...
!			 if(ink==nelec) then
!			     do j=1,12
!			         deltarm=deltarm+(his(57+j,ink,nintg)-abc(57+j,ink,nintg))
!					 deltarim=deltarim+(his(81+j,ink,nintg)-abc(81+j,ink,nintg))
!			     end do
!				 do j=1,6
!				     deltarim=deltarim+(his(250+j,ink,nintg)-abc(250+j,ink,nintg))
!				 end do
!				 write(2001,*) deltarm
!				 write(2002,*) deltarim
!				 write(2003,*) qppower
...
!!!!!$OMP PARALLEL DO       
		 ...
		 ! =========================================================
            ...
            p(j,3) = 0.5 * (slip_s(j,2)*slip_n(j,3) +
     >						    slip_s(j,3)*slip_n(j,2))    !12 plastic symmetric
            p(j,4)=(slip_s(j,1)*slip_n(j,1))                !33 plastic symmetric
            w12(j) = 0.5 * (slip_s(j,2)*slip_n(j,3) -                                  !These 2 Assignments: Wij,  Zikry & Nasser, 1990 pg 217
     >						 slip_s(j,3)*slip_n(j,2))       !plastic asymmetric
...
!!!!!!!!$OMP PARALLEL DO       
		 ...
		 ! ========		
            Pij_Dijdev(j) = p(j,1)*Dij_dev(1) +   p(j,2)*Dij_dev(2)		!This loop: Pij(alpha)*Dij',  Zikry & Nasser, 1990 pg 218
     >				      + 2*p(j,3)*Dij_dev(4)                         !Calculates time derivative of resolved shear stress
     >                    +  p(j,4)*Dij_dev(3)                          !UPDATED FOR (3,3) term, WML 9/19/08 FOR OLD WAY, JUST COMMENT OUT
		!-------------------------------------------------------------
         end do
...
!c             tau(j)  = p(j,1)*sdev1 + p(j,2)*sdev2 + 2*p(j,3)*sdev4		!This Line: Cauchy Tau(alpha), Zikry & Nasser, 1990 pg 217   commented out b/c using dsolve
		     ...
		     ! ======================	 
             gdot(j) = ref_gamma_dot(j)*									!This Line: strain rate(alpha), Zikry & Nasser, 1990 pg 217
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     >              +ssdev3*D33_p                       !REMOVED ABS Values WML 9/19/08
           ...
           DijSij_e = ssdev1*(Dij_dev(1)-D11_p)    + 
     >                ssdev2*(Dij_dev(2)-D22_p)    +		       !qwu
     ...
     >   Dij_dev(2)+Dij_dev(3)*Dij_dev(3)+2.*Dij_dev(4)*Dij_dev(4))))
           ...
           if ((Dij_dev(1)*Dij_dev(1)+Dij_dev(2)*
     >         Dij_dev(2)+Dij_dev(3)*Dij_dev(3)+
     >         2.*Dij_dev(4)*Dij_dev(4)).eq.0.) then
               effmod(i)=prop(10,matp(ink))
           end if
!c      intener(i)=(sdev4+twomu*dt*(Dij_dev(4)-D12_p)*2)*
!c     1  (Dij_dev(4))+(press+sdev1+twomu*dt*
!c     2  (Dij_dev(1)-D11_p+traced/3.))*
!c     3  (Dij_dev(1)+traced/3.)+(press+sdev2+twomu*dt*
!c     4(Dij_dev(2)-D22_p+traced/3.))*
!c     5  (Dij_dev(2)+traced/3.)+(press+sdev3+twomu*dt*
!c     6(Dij_dev(3)-D33_p+traced/3.))*
!c     7  (Dij_dev(3)+traced/3.)

           if (effmod(i).lt.0.) effmod(i)=0.
 !        DijSij = abs(ssdev1*D11_p) + abs(ssdev2*D22_p)     +		    !This Line: StressPower, Zikry: 1992-1993, pg 275
 !    >                                abs(2.00*ssdev4*D12_p)    OLD WAY
...
!c		       call elastic_energy(i, qe)
		       inie(ink)=exp(-(abs(qe/qp))/10000.0)
			   ecount(ink)=1
!			   write(*,*) 'decay', ink, qe, qp, qe/qp, inie(ink)
		   end if
!c		   else
...
!!!!!!	     sigalt(4,ink) = sig(4,i)
         ...
         intener(i)=sig(1,i)*d1(i)+sig(2,i)*d2(i)+sig(3,i)*d3(i)+
     >              2.*sig(4,i)*d4(i)
