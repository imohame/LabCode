...
module CN_Consts
    ...
    real*8 :: sqr3_3=1.73205080756888/3.d0 !sqrt(3.d0)/3.d0
...
!!!        ---------------------------------------
        subroutine CNCalcElemAverageFromNodes (ElemCount, NodeCount,ElemConnect,rNodesIn,rElemOut)
            ...
            integer,    intent(in) :: ElemConnect(4,ElemCount)
            ...
            do i=1,ElemCount
                Valn1=rNodesIn(ElemConnect(1,i))
                Valn2=rNodesIn(ElemConnect(2,i))
                Valn3=rNodesIn(ElemConnect(3,i))
                Valn4=rNodesIn(ElemConnect(4,i))
...
!!!        --------------------------------------- to clean the memory for the Rn's and CTn's
        subroutine CN_Base_ForceBCs (tCN_object,RHS2,LHS,rTn)
            use CN_Consts,only:NodeCountAct
            implicit none
            class ( CN_Base ), intent(inout) :: tCN_object
            real*8,    intent(in) :: rTn(NodeCountAct)
            real*8 ,    intent(inout) ::RHS2(NodeCountAct,1),LHS(NodeCountAct,NodeCountAct)
            integer NodeID,i
            ...
            do i=1,tCN_object%iBCcount
                ...
                LHS(NodeID,1:NodeCountAct)=0.
                LHS(NodeID,NodeID)=1.0
                RHS2(NodeID,1)=rTn(NodeID)
!!!!                write(*,*)NodeID,rTn(NodeID)
            enddo
...
!!!        --------------------------------------- to clean the memory for the Rn's and CTn's
        subroutine CNBase_InitiateTn_1 (obj,ElemConnect)
            ...
            integer,    intent(in) :: ElemConnect(4,ElemCountAct)
...
!      to apply the effect of thermal loading  tCN_object%rBCTempConc(NodeID) rTn_1
            call obj%CN_Base_ForceBCs (RHS2,LHS,obj%rBCTempConc)
...
!!!!            write(*,26) obj%rThermal_Diff_Ele(1:ElemCountAct)
!!!!            26 format(e15.6)
!!!!            stop
