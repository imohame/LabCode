...
subroutine FractReadApplyPreCrackCleavagePlanes(NodesCoordx,NodesCoordy,ElemConnect,ElemMaterial,DofIds &
    ...
    call EC_AllocateMem(ElemConnect,NodesCoordx,NodesCoordy)

!--------      this is has to be called first b/c the pre-crack needs it
    call FractReadElemNeighbors()
    !- write iFU_crackinput_out file to check the input data of the elem
    call EC_PrintTest()
    close(iFU_crackinput_out)
    call EC_CalcElemNeighbors(tempInt,0,1)
!!! if the fracture is not active then exit
    if (mEC_PreCrackedElemCount == 0) then
        close(60)
        close(6011)
...
! !###################################################################
 subroutine FractReadElemNeighbors()
    ...
    open(70, file = 'ElemsNeighbors.in', status = 'old',IOSTAT=IERR, ERR=90)
    open(7011, file = 'ElemsNeighbors2.out', status = 'unknown')
    ierr=setvbuf3f_local(7011,1,100)

    do i = 1, EC_ElemCountInput

      read (70, *)    EC_ElemNeighbors(1:8)
      write (7011, *)    EC_ElemNeighbors(1:8)
